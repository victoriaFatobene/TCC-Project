...
app.post('/items', async(req,res) => {
    ...
    try{
        ...
        res.status(500).json({error: 'Erro ao adcionar o item'});
    }
});
...
app.post('/item-modifications', async (req, res) => {
    ...
    try{
        ...
        res.status(500).json({error: 'Error ao criat a modificação de item'});
    }
});
...
app.post('/product-ingredients', async (req, res) => {
    ...
    try{
        ...
        res.status(201).json(newPrdoductIngredient);
    }catch (error){
...
app.post('/api/reviews', async(req, res) => {
    try{
        ...
        if (!userId || !productId || !rating) {
            return res.status.apply(400).json({error: 'Campos obrigatórios pendentes: userId, productId e rating'});
        }
